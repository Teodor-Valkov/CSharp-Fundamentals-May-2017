namespace _11.SemanticHtml
{
    ...
    internal class SemanticHtml
    {
        private static void Main()
        {
            ...
            Regex opening = new Regex(openingTags);

            string closeTagsPattern = @"<\/div>\s.*?(\w+)\s*-->";
            Regex closing = new Regex(closeTagsPattern);
            ...
            {
                Match openingMatch = opening.Match(input);
                ...
                Match commentsMatch = closing.Match(input);
