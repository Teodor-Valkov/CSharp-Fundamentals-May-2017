namespace _06.ZippingSlicedFiles
{
    ...
    internal class StartUp
    {
        ...
        private static void Main()
        {
            ...
            Console.WriteLine("*** 5 Compressed parts were created from the source file - 'input.txt'!");
            Console.WriteLine($"*** 1 Decompressed assembled file was created from the 5 Compressed parts!{Environment.NewLine}");
        }
        ...
        private static void Slice(string sourceFile, string destinationDirectory, int parts)
        {
            using (FileStream reader = new FileStream(sourceFile, FileMode.Open))
            {
                ...
                {
                    string currentPartFileName = $"{destinationDirectory}Part-{i}.gz";
        ...
        private static void Assemble(List<string> files, string destinationDirectory)
        {
            ...
            using (writer = new FileStream(assemledFile, FileMode.Append))
            {
                ...
                foreach (string filePart in files)
                {
                    ...
                    {
                        using (GZipStream decompressionStream = new GZipStream(reader, CompressionMode.Decompress, false))
                        {
                            ...
                            while (true)
                            {
                                int readBytes = decompressionStream.Read(byteBuffer, 0, byteBuffer.Length);
                                ...
                                writer.Write(byteBuffer, 0, readBytes);
                            }
                        }
                    }
                }
